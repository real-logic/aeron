...
int aeron_mpsc_concurrent_array_queue_init(aeron_mpsc_concurrent_array_queue_t *queue, uint64_t length)
{
...
extern uint64_t aeron_mpsc_concurrent_array_queue_drain(
    aeron_mpsc_concurrent_array_queue_t *queue, aeron_queue_drain_func_t func, void *clientd, uint64_t limit);

extern uint64_t aeron_mpsc_concurrent_array_queue_drain_all(
    aeron_mpsc_concurrent_array_queue_t *queue, aeron_queue_drain_func_t func, void *clientd);

extern uint64_t aeron_mpsc_concurrent_array_queue_size(aeron_mpsc_concurrent_array_queue_t *queue);
