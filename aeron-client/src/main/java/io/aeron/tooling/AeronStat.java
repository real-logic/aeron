...
import static io.aeron.AeronCounters.DRIVER_SEND_CHANNEL_STATUS_TYPE_ID;
...
public class AeronStat
{
    ...
    static class CounterFilter
    {
        ...
        boolean filter(final int typeId, final DirectBuffer keyBuffer)
        {
            ...
            if (DRIVER_SYSTEM_COUNTER_TYPE_ID == typeId &&
                !match(identityFilter, () -> Integer.toString(keyBuffer.getInt(0))))
            {
                return false;
            }
            else if ((typeId >= DRIVER_PUBLISHER_LIMIT_TYPE_ID && typeId <= DRIVER_RECEIVER_POS_TYPE_ID) ||
            ...
            else if (typeId >= DRIVER_SEND_CHANNEL_STATUS_TYPE_ID && typeId <= DRIVER_RECEIVE_CHANNEL_STATUS_TYPE_ID)
            {
