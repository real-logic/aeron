...
void aeron_driver_conductor_cleanup_spies(aeron_driver_conductor_t *conductor, aeron_network_publication_t *publication)
{
    ...
    for (size_t i = 0, size = conductor->spy_subscriptions.length; i < size; i++)
    {
        ...
        aeron_driver_conductor_unlink_subscribeable(link, &publication->conductor_fields.subscribeable);
    }
}
            ...
            if (ensure_capacity_result >= 0)
            {
                ...
                snd_pos_position.counter_id =
                    aeron_counter_sender_position_allocate(
                        &conductor->counters_manager, registration_id, session_id, stream_id, channel);
                snd_lmt_position.counter_id =
                    aeron_counter_sender_limit_allocate(
                        &conductor->counters_manager, registration_id, session_id, stream_id, channel);
                ...
                snd_lmt_position.value_addr =
                    aeron_counter_addr(&conductor->counters_manager, (int32_t)snd_lmt_position.counter_id);
    ...
    aeron_driver_conductor_t *conductor, aeron_udp_channel_t *channel)
{
    ...
    if (NULL == endpoint)
    {
        ...
        status_indicator.counter_id =
            aeron_counter_receive_channel_status_allocate(&conductor->counters_manager, channel->original_uri);
...
void aeron_driver_conductor_on_close(void *clientd)
{
    ...
    {
        aeron_free(conductor->network_subscriptions.array[i].subscribeable_list.array);
    }
    ...
    for (size_t i = 0, length = conductor->spy_subscriptions.length; i < length; i++)
    {
        ...
        aeron_free(conductor->spy_subscriptions.array[i].subscribeable_list.array);
    }
    ...
    size_t log_file_name_length)
{
    ...
    if (ensure_capacity_result >= 0)
    {
        ...
        if (counter_id >= 0)
        {
            ...
            if (aeron_driver_subscribeable_add_position(subscribeable, counter_id, position_addr) >= 0)
            {
    ...
    for (size_t i = 0; i < conductor->spy_subscriptions.length; i++)
    {
            ...
            !aeron_driver_conductor_is_subscribeable_linked(subscription_link, subscribeable))
        {
            if (aeron_driver_conductor_link_subscribeable(
                conductor,
                subscription_link,
                &publication->conductor_fields.subscribeable,
                publication->conductor_fields.managed_resource.registration_id,
                publication->session_id,
                publication->stream_id,
                aeron_network_publication_spy_join_position(publication),
                endpoint->conductor_fields.udp_channel->original_uri,
...
void aeron_driver_conductor_on_create_publication_image(void *clientd, void *item)
{
    ...
    rcv_hwm_position.counter_id =
        aeron_counter_receiver_hwm_allocate(
            &conductor->counters_manager, registration_id, command->session_id, command->stream_id, channel_str);
    rcv_pos_position.counter_id =
        aeron_counter_receiver_position_allocate(
            &conductor->counters_manager, registration_id, command->session_id, command->stream_id, channel_str);
    ...
    rcv_hwm_position.value_addr =
        aeron_counter_addr(&conductor->counters_manager, (int32_t)rcv_hwm_position.counter_id);
    rcv_pos_position.value_addr =
        aeron_counter_addr(&conductor->counters_manager, (int32_t)rcv_pos_position.counter_id);
