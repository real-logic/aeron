/*
 * Copyright 2014-2024 Real Logic Limited.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * https://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef AERON_DRIVER_EXECUTOR_H
#define AERON_DRIVER_EXECUTOR_H

#include "aeron_driver_common.h"
#include "concurrent/aeron_blocking_linked_queue.h"

typedef struct aeron_driver_executor_task_stct aeron_driver_executor_task_t;

typedef int (*aeron_driver_executor_on_execution_complete_func_t)(aeron_driver_executor_task_t *task, void *executor_clientd);

typedef struct aeron_driver_executor_stct
{
    aeron_blocking_linked_queue_t queue;
    aeron_driver_executor_on_execution_complete_func_t on_execution_complete;
    void *clientd;
}
aeron_driver_executor_t;

int aeron_driver_executor_init(
    aeron_driver_executor_t *executor,
    aeron_driver_executor_on_execution_complete_func_t on_execution_complete,
    void *clientd);

int aeron_driver_executor_close(aeron_driver_executor_t *executor);

int aeron_driver_executor_do_work(aeron_driver_executor_t *executor);

typedef int (*aeron_driver_executor_task_on_execute_func_t)(void *task_clientd, void *executor_clientd);

typedef int (*aeron_driver_executor_task_on_complete_func_t)(int execution_result, void *task_clientd, void *executor_clientd);

int aeron_driver_executor_execute(
    aeron_driver_executor_t *executor,
    aeron_driver_executor_task_on_execute_func_t on_execute,
    aeron_driver_executor_task_on_complete_func_t on_complete,
    void *clientd);

int aeron_driver_executor_task_do_complete(aeron_driver_executor_task_t *task);

#endif //AERON_DRIVER_EXECUTOR_H
