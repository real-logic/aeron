...
typedef struct aeron_tagged_flow_control_strategy_receiver_stct
{
...
aeron_tagged_flow_control_strategy_receiver_t;
...
typedef struct aeron_tagged_flow_control_strategy_state_stct
{
    struct receiver_stct
    {
        size_t length;
        size_t capacity;
        aeron_tagged_flow_control_strategy_receiver_t *array;
    }
    receivers;
    ...
    bool is_end_of_stream)
{
    ...
    {
        aeron_tagged_flow_control_strategy_receiver_t *receiver = &strategy_state->receivers.array[i];
        ...
        if ((receiver->time_of_last_status_message + strategy_state->receiver_timeout_ns) - now_ns < 0)
        {
            aeron_array_fast_unordered_remove(
                ...
                sizeof(aeron_tagged_flow_control_strategy_receiver_t),
                (size_t)i,
                (size_t)last_index);
            last_index--;
    ...
    return (int32_t)strategy_state->receivers.length < strategy_state->group_min_size ||
...
int64_t aeron_tagged_flow_control_strategy_handle_sm(
    aeron_tagged_flow_control_strategy_state_t *strategy_state,
    const aeron_status_message_header_t *status_message_header,
    const bool is_tagged,
    const size_t length,
    const struct sockaddr_storage *recv_addr,
    const int64_t snd_lmt,
    const int32_t initial_term_id,
    const size_t position_bits_to_shift,
    const int64_t now_ns)
{
    ...
    if ((int32_t)strategy_state->receivers.length < strategy_state->group_min_size)
    {
        return snd_lmt;
    }
    ...
    int64_t now_ns)
{
    ...
    return aeron_tagged_flow_control_strategy_handle_sm(
        strategy_state,
    ...
    size_t term_buffer_capacity)
{
    ...
    aeron_tagged_flow_control_strategy_state_t *state = (aeron_tagged_flow_control_strategy_state_t *)_strategy->state;

    state->receivers.array = NULL;
    state->receivers.capacity = 0;
    aeron_min_flow_control_strategy_state_set_length(state, 0);
    state->group_receiver_tag = -1;
