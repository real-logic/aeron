    ...
    int64_t blocked_position)
{
    ...
    const size_t index = aeron_logbuffer_index_by_position(blocked_position, position_bits_to_shift);
        ...
        log_meta_data, term_buffers[index].addr, term_length, blocked_offset, tail_offset, term_id))
    {
        case AERON_TERM_UNBLOCKER_STATUS_UNBLOCKED_TO_END:
        {
            ...
            aeron_logbuffer_rotate_log(log_meta_data, term_count, term_id);
        }
            /* fall through */
