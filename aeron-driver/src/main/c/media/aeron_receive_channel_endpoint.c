    ...
    aeron_receive_destination_t *destination)
{
    ...
    if (destination->conductor_fields.udp_channel->has_explicit_control)
    {
        ...
        {
            aeron_set_err(EINVAL, "receiver on_add_endpoint: %s", aeron_errmsg());
            return -1;
        }
        ...
        {
            aeron_set_err(EINVAL, "aeron_receive_channel_endpoint_add_pending_setup send SM: %s", aeron_errmsg());
            return -1;
        }
