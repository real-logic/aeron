...
public:
    ...
    {
        m_buffer.fill(0);
        aeron_stub_clock_set_time_ms(7);
    }
...
TEST_F(DistinctErrorLogTest, shouldFailToRecordWhenInsufficientSpace)
{
    ...
    ASSERT_EQ(aeron_distinct_error_log_init(&m_log, buffer.data(), buffer.size(), linger_resource, NULL), 0);
...
{
    ASSERT_EQ(aeron_distinct_error_log_init(&m_log, m_buffer.data(), m_buffer.size(), linger_resource, NULL), 0);
...
TEST_F(DistinctErrorLogTest, shouldSummariseObservations)
{
    ...
    EXPECT_EQ(aeron_distinct_error_log_record(&m_log, 1, "description", "message"), 0);
    aeron_stub_clock_increment_ms(1);
    EXPECT_EQ(aeron_distinct_error_log_record(&m_log, 1, "description", "message"), 0);
