...
{
    aeron_untethered_subscription_state_change_func_t func = m_context->log.untethered_subscription_on_state_change_func;
    ...
    EXPECT_NE(m_context->log.untethered_subscription_on_state_change_func, func);
}
...
TEST_F(DriverAgentTest, shouldKeepOriginalUntetheredStateChangeFunctionIfEventNotEnabled)
{
    ...
    EXPECT_EQ(m_context->log.untethered_subscription_on_state_change_func, func);
}
...
{
    aeron_driver_name_resolver_on_neighbor_change_func_t func = m_context->log.name_resolution_on_neighbor_added_func;

    EXPECT_TRUE(aeron_driver_agent_logging_events_init("NAME_RESOLUTION_NEIGHBOR_ADDED", nullptr));
    aeron_driver_agent_init_logging_events_interceptors(m_context);

    EXPECT_NE(m_context->log.name_resolution_on_neighbor_added_func, func);
}
...
TEST_F(DriverAgentTest, shouldKeepOriginalNameResolutionOnNeighborAddedFunctionIfEventNotEnabled)
{
    ...
    EXPECT_EQ(m_context->log.name_resolution_on_neighbor_added_func, func);
}

TEST_F(DriverAgentTest, shouldInitializeNameResolutionOnNeighborRemovedInterceptor)
{
    aeron_driver_name_resolver_on_neighbor_change_func_t func = m_context->log.name_resolution_on_neighbor_removed_func;

    EXPECT_TRUE(aeron_driver_agent_logging_events_init("NAME_RESOLUTION_NEIGHBOR_REMOVED", nullptr));
    aeron_driver_agent_init_logging_events_interceptors(m_context);

    EXPECT_NE(m_context->log.name_resolution_on_neighbor_removed_func, func);
}
...
TEST_F(DriverAgentTest, shouldKeepOriginalNameResolutionOnNeighborRemovedFunctionIfEventNotEnabled)
{
    ...
    EXPECT_EQ(m_context->log.name_resolution_on_neighbor_removed_func, func);
}
...
{
    const aeron_on_remove_publication_cleanup_func_t func = m_context->log.remove_publication_cleanup_func;

    EXPECT_TRUE(aeron_driver_agent_logging_events_init("REMOVE_PUBLICATION_CLEANUP", nullptr));
    aeron_driver_agent_init_logging_events_interceptors(m_context);

    EXPECT_NE(m_context->log.remove_publication_cleanup_func, func);
}
...
{
    const aeron_on_remove_subscription_cleanup_func_t func = m_context->log.remove_subscription_cleanup_func;

    EXPECT_TRUE(aeron_driver_agent_logging_events_init("REMOVE_SUBSCRIPTION_CLEANUP", nullptr));
    aeron_driver_agent_init_logging_events_interceptors(m_context);

    EXPECT_NE(m_context->log.remove_subscription_cleanup_func, func);
}
...
{
    const aeron_on_remove_image_cleanup_func_t func = m_context->log.remove_image_cleanup_func;

    EXPECT_TRUE(aeron_driver_agent_logging_events_init("REMOVE_IMAGE_CLEANUP", nullptr));
    aeron_driver_agent_init_logging_events_interceptors(m_context);

    EXPECT_NE(m_context->log.remove_image_cleanup_func, func);
}
...
{
    const aeron_on_endpoint_change_func_t func = m_context->log.sender_proxy_on_add_endpoint_func;

    EXPECT_TRUE(aeron_driver_agent_logging_events_init("SEND_CHANNEL_CREATION", nullptr));
    aeron_driver_agent_init_logging_events_interceptors(m_context);

    EXPECT_NE(m_context->log.sender_proxy_on_add_endpoint_func, func);
}

TEST_F(DriverAgentTest, shouldInitializeSendChannelCloseInterceptor)
{
    const aeron_on_endpoint_change_func_t func = m_context->log.sender_proxy_on_remove_endpoint_func;

    EXPECT_TRUE(aeron_driver_agent_logging_events_init("SEND_CHANNEL_CLOSE", nullptr));
    aeron_driver_agent_init_logging_events_interceptors(m_context);

    EXPECT_NE(m_context->log.sender_proxy_on_remove_endpoint_func, func);
}

TEST_F(DriverAgentTest, shouldInitializeReceiveChannelCreationInterceptor)
{
    const aeron_on_endpoint_change_func_t func = m_context->log.receiver_proxy_on_add_endpoint_func;

    EXPECT_TRUE(aeron_driver_agent_logging_events_init("RECEIVE_CHANNEL_CREATION", nullptr));
    aeron_driver_agent_init_logging_events_interceptors(m_context);

    EXPECT_NE(m_context->log.receiver_proxy_on_add_endpoint_func, func);
}

TEST_F(DriverAgentTest, shouldInitializeReceiveChannelCloseInterceptor)
{
    const aeron_on_endpoint_change_func_t func = m_context->log.receiver_proxy_on_remove_endpoint_func;

    EXPECT_TRUE(aeron_driver_agent_logging_events_init("RECEIVE_CHANNEL_CLOSE", nullptr));
    aeron_driver_agent_init_logging_events_interceptors(m_context);

    EXPECT_NE(m_context->log.receiver_proxy_on_remove_endpoint_func, func);
}
...
{
    aeron_driver_flow_control_strategy_on_receiver_change_func_t func = m_context->log.flow_control_on_receiver_added_func;
    EXPECT_EQ(nullptr, func);

    EXPECT_TRUE(aeron_driver_agent_logging_events_init("FLOW_CONTROL_RECEIVER_ADDED", nullptr));
    aeron_driver_agent_init_logging_events_interceptors(m_context);

    EXPECT_NE(nullptr, m_context->log.flow_control_on_receiver_added_func);
}
...
TEST_F(DriverAgentTest, shouldNotAssignFlowControlOnReceiverAddedInterceptorWhenNotConfigured)
{
    ...
    EXPECT_EQ(nullptr, m_context->log.flow_control_on_receiver_added_func);
}
...
{
    aeron_driver_flow_control_strategy_on_receiver_change_func_t func = m_context->log.flow_control_on_receiver_removed_func;
    EXPECT_EQ(nullptr, func);

    EXPECT_TRUE(aeron_driver_agent_logging_events_init("FLOW_CONTROL_RECEIVER_REMOVED", nullptr));
    aeron_driver_agent_init_logging_events_interceptors(m_context);

    EXPECT_NE(nullptr, m_context->log.flow_control_on_receiver_removed_func);
}
...
TEST_F(DriverAgentTest, shouldNotAssignFlowControlOnReceiverRemovedInterceptorWhenNotConfigured)
{
    ...
    EXPECT_EQ(nullptr, m_context->log.flow_control_on_receiver_removed_func);
}
...
TEST_F(DriverAgentTest, shouldNotAssignOnNameResolveFunctionWhenNotConfigured)
{
    ...
    EXPECT_EQ(nullptr, m_context->log.on_name_resolve_func);
}
...
TEST_F(DriverAgentTest, shouldNotAssignOnNameLookupFunctionWhenNotConfigured)
{
    ...
    EXPECT_EQ(nullptr, m_context->log.on_name_lookup_func);
}
...
TEST_F(DriverAgentTest, shouldNotHostNameFunctionWhenNotConfigured)
{
    ...
    EXPECT_EQ(nullptr, m_context->log.on_host_name_func);
}
...
TEST_F(DriverAgentTest, shouldInitializeOnNameResolveFunction)
{
    ...
    EXPECT_NE(nullptr, m_context->log.on_name_resolve_func);
}
...
TEST_F(DriverAgentTest, shouldInitializeOnNameLookupFunction)
{
    ...
    EXPECT_NE(nullptr, m_context->log.on_name_lookup_func);
}
...
TEST_F(DriverAgentTest, shouldInitializeNameResolverFunctions)
{
    ...
    EXPECT_NE(nullptr, m_context->log.on_name_resolve_func);
    EXPECT_NE(nullptr, m_context->log.on_name_lookup_func);
    EXPECT_NE(nullptr, m_context->log.on_host_name_func);
    EXPECT_NE((void *)m_context->log.on_name_resolve_func, (void *)m_context->log.on_name_lookup_func);
}
...
TEST_F(DriverAgentTest, shouldNotSendNakMessageFunctionWhenNotConfigured)
{
    ...
    EXPECT_EQ(nullptr, m_context->log.send_nak_message_func);
}
...
TEST_F(DriverAgentTest, shouldInitializeSendNakMessageFunction)
{
    ...
    EXPECT_NE(nullptr, m_context->log.send_nak_message_func);
}
...
TEST_F(DriverAgentTest, shouldNotRsendFunctionWhenNotConfigured)
{
    ...
    EXPECT_EQ(nullptr, m_context->log.resend_func);
}
...
TEST_F(DriverAgentTest, shouldInitializeResendFunction)
{
    ...
    EXPECT_NE(nullptr, m_context->log.resend_func);
}
