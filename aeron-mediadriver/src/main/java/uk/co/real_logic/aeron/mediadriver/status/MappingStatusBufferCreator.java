 ...
 */
package uk.co.real_logic.aeron.mediadriver.status;
...
import static uk.co.real_logic.aeron.mediadriver.MediaDriver.COUNTERS_BUFFER_SZ;
import static uk.co.real_logic.aeron.mediadriver.MediaDriver.DESCRIPTOR_BUFFER_SZ;
import static uk.co.real_logic.aeron.util.IoUtil.mapNewFile;

/**
 * .
 */
public class MappingStatusBufferCreator implements AutoCloseable
{
    ...
    public MappingStatusBufferCreator()
    {
        final File directory = new File(CommonConfiguration.COUNTERS_DIR);
        try
        {
            descriptor = mapNewFile(directory, "descriptor", DESCRIPTOR_BUFFER_SZ);
            counter = mapNewFile(directory, "counter", COUNTERS_BUFFER_SZ);
