...
Firstly, some known initial state, this could either be the state when first provisioned (a 'null' or empty state) or
...
Iterating over collections should happen in a known order, with a collection that preserves ordering (e.g. TreeMap,
...
The MediaDriver is the means by which data is sent into, out of, and around the cluster. Aeron Cluster reuses the
Publication and Subscription functionality in Aeron to handle all distributed and inter-process messaging.
...
a simple auction service. It will have one auction and will track the best price, and an id for the customer who bid
that price. To properly demonstrate the state management and recovery features of cluster it is important to have some
...
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredService.java[tag=new_service]
----
...
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredService.java[tag=start]
----
...
<3> The snapshot can be null (this occurs the first time that the service is started).
...
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredService.java[tag=state]
----
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredService.java[tag=message]
----
...
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredService.java[tag=takeSnapshot]
----
...
the Cluster's `IdleStrategy::idle` inside of the busy wait loop.
...
detected from the store messages. The two approaches can be used to sanity check all the data is received.

[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredService.java[tag=loadSnapshot]
----
...
<4> Once all the data is loaded, we can initialise the application logic state from the snapshot.
...
(MediaDriver, Archive, ConsensusModule and ClusteredServiceContainer) for a single node are hosted in a
single process. Later we will then start the cluster as three separate processes.
...
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredServiceNode.java[tag=ports]
----
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredServiceNode.java[tag=udp_channel]
----
...
To start the cluster node we are going define our own class has a main method, it will construct all contexts for the
necessary components and then start them. As indicated above we are going to give each node of the cluster a unique id
(0, 1, and 2). We are also going to use this value as the cluster member id.

[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredServiceNode.java[tag=new_service]
----
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredServiceNode.java[tag=main]
----
...
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredServiceNode.java[tag=media_driver]
----
...
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredServiceNode.java[tag=archive]
----
...
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredServiceNode.java[tag=archive_client]
----
...
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredServiceNode.java[tag=consensus_module]
----
...
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredServiceNode.java[tag=clustered_service]
----
...
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusteredServiceNode.java[tag=running]
----
...
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusterClient.java[tag=client]
----
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusterClient.java[tag=connect]
----
...
will need to be in environment where multicast is supported. In cloud deployments UDP multicast is typically not
...
[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusterClient.java[tag=publish]
----
...
messages), or notifications of a new leader. In this tutorial there is not a lot to do, we simple print out the
responses that we receive.

[source, Java, indent=0, options="nowrap"]
----
include::{sampleSourceDir}/io/aeron/samples/cluster/BasicAuctionClusterClient.java[tag=response]
----
...
We can trigger a snapshot on the leader. This is slightly different to using the `ClusterTool` in that the
`ClusterControl` needs the Aeron directory for the node's media driver.

----
> java -cp ../../../aeron-all/build/libs/aeron-all-{aeronVersion}.jar \
