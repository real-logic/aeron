...
provides a means to persist an Aeron stream is used by Aeron Cluster to persist the Raft log.  Messages are sent around
a cluster using Aeron's Media Driver. Therefore, Aeron Cluster is an aggregation of a number of existing Aeron
...
The MediaDriver is the means by which data is moved into, out of, and around the cluster. Aeron Cluster reuses the
...
a simple auction service. It will have one auction and will track a best price, and an id for the customer who bid that
...
code (data serialisation, application logic, Cluster integration) very close together. It is unlikely you would do
...
<5> Calling `offer` on the client session will send the response on the egress channel. Make sure the return value
...
wait loop to allow the service to pause in a friendly manner. It will take care of handling thread interrupts and
ensure the node fails correctly.
...
logic's state. For real world applications snapshotting can become tricky. The two big concerns you will have are to
...
Our `onStart` implementation contained a method to load the snapshot. Now we have seen how the snapshot is written, we
can look at how it is loaded. The `Image` provided to the `onStart` has a method that will indicate if there is no more
data available. However, application code should also encode enough information that the end of the stream can be
detected from the store messages. The two approaches can be used to do sanity checking that all the data is received.
...
Now the cluster is configured we can start it running. The code for launching the service is as follows.
...
to allow for multiple clients on the same machine with multiple media drivers. If a single media driver is used then
...
available for Cluster and experiment with some failover functionality.

First thing we are interested in is which node of the three is the leader, with this information we can try to
shutdown the leader and see if our client is able to recover.
...
The client should automatically resumed sending messages into the cluster and receiving responses. There may or may
...
determining the size of the individual Aeron message been sent. Given that we will be handling fragmentation ourselves,
