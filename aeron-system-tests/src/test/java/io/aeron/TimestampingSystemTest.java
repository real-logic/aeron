...
class TimestampingSystemTest
{
    ...
    @Test
    @InterruptAfter(10)
    void shouldSupportReceiveTimestampsOnMergedMds2()
    {
        final MutableDirectBuffer buffer = new UnsafeBuffer(new byte[64]);
        ...
        try (TestMediaDriver driver = driver();
            Aeron aeron = Aeron.connect(new Aeron.Context().aeronDirectoryName(driver.aeronDirectoryName())))
        {
            final Subscription mdsSub = aeron.addSubscription("aeron:udp?control-mode=manual", 1000);
            ...
            while (!pub1.isConnected() || !pub2.isConnected() || !mdsSub.isConnected())
            {
                Tests.yieldingIdle("Failed to connect");
            }
